...
/*
【正则表达式】
Java 8 在 java.util.regex.Pattern 中增加了一个新的方法 splitAsStream()。
这个方法可以根据传入的公式将字符序列转化为流。

但是有一个限制，
输入只能是 CharSequence（因此不能将流作为 splitAsStream() 的参数）
...
class FileToWordsRegexp {
    ...
    public FileToWordsRegexp() {
        all = "Not much of a cheese shop really, is it?";
    }
...
/*
这个例子不同的地方在于
可以多次调用 stream() 在已存储的字符串中创建一个新的流（这时一个优点）。

但同时也有限制，整个字符串都创建在内存中。
在大多数情况下这并不是什么问题。
但如果是使用文件获取流的话（像这样：FileToWordsRegexp(String filePath)），
损失了流操作非常重要的优势：
1.流“不需要存储”。
    当然它们需要一些内部存储，但是这只是序列的一小部分，和持有整个序列并不相同。
2.流是懒加载计算的。
