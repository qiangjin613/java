 ...
 * （重载的 +、+= 就是一个例子）
 *
 * 用于 String 的 + 与 += 是 Java 中仅有的两个重载过的操作符，
...
class WitherStringBuilder {
    // [2] 编译器的优化不是完美的
    // StringBuilder 是在循环内构造的，这意味着每进行一次循环，会创建一个新的 StringBuilder 对象。
    ...
    // 编译器只生成一个 StringBuilder 对象，显示创建还允许预先为 StringBuilder 指定大小（可以避免频繁地重新分配缓冲）
...
class UsingStringBuilder {
    public static String string1() {
        ...
        for (int i = 0; i < 25; i++) {
            ...
            result.append(", ");
            // 这样的“捷径”会让编译器调入陷阱，从而创建另一个 StringBuilder 处理括号内的字符串操作
            // result.append(rand.nextInt(100) + ", ");
        }
